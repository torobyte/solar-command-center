...
/** Variante FLOW — Solar → Batería → Carga con flechas y pulsos animados. */
class SolarOpsWidgetFlow : AppWidgetProvider() {
    ...
    override fun onEnabled(context: Context) {
        ...
        WidgetCommon.scheduleAlarmFor(context, SolarOpsWidgetFlow::class.java)
    }

    override fun onDisabled(context: Context) {
        super.onDisabled(context)
        WidgetCommon.cancelAlarmFor(context, SolarOpsWidgetFlow::class.java)
    }
    ...
    private fun refreshAll(context: Context) {
        ...
        for (id in WidgetCommon.widgetIdsFor(context, SolarOpsWidgetFlow::class.java))
            refresh(context, mgr, id)
    }

    private fun refresh(context: Context, mgr: AppWidgetManager, widgetId: Int) {
        val views = RemoteViews(context.packageName, R.layout.widget_solarops_flow)
        ...
        if (token.isNullOrBlank()) {
            views.setTextViewText(R.id.flow_site, "Configura el token")
    ...
    private fun apply(mgr: AppWidgetManager, id: Int, v: RemoteViews, json: JSONObject?) {
        if (json == null) {
            v.setTextViewText(R.id.flow_site, "Sin conexión")
        ...
        v.setTextViewText(R.id.flow_site, site?.optString("name") ?: "SolarOps · Flow")
        v.setTextViewText(R.id.flow_pv, "${pv}W")
        v.setTextViewText(R.id.flow_bat, "${bat}%")
        v.setTextViewText(R.id.flow_load, "${load}W")
        v.setTextViewText(R.id.flow_updated, "● en vivo")
