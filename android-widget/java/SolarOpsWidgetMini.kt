...
/** Variante MINI — tarjeta 2x1 con % de batería gigante y pulso animado. */
class SolarOpsWidgetMini : AppWidgetProvider() {
    ...
    override fun onEnabled(context: Context) {
        ...
        WidgetCommon.scheduleAlarmFor(context, SolarOpsWidgetMini::class.java)
    }

    override fun onDisabled(context: Context) {
        super.onDisabled(context)
        WidgetCommon.cancelAlarmFor(context, SolarOpsWidgetMini::class.java)
    }
    ...
    private fun refreshAll(context: Context) {
        ...
        for (id in WidgetCommon.widgetIdsFor(context, SolarOpsWidgetMini::class.java))
            refresh(context, mgr, id)
    }

    private fun refresh(context: Context, mgr: AppWidgetManager, widgetId: Int) {
        val views = RemoteViews(context.packageName, R.layout.widget_solarops_mini)
        ...
        if (token.isNullOrBlank()) {
            views.setTextViewText(R.id.mini_site, "Sin token")
    ...
    private fun apply(mgr: AppWidgetManager, id: Int, v: RemoteViews, json: JSONObject?) {
        if (json == null) {
            v.setTextViewText(R.id.mini_site, "Sin conexión")
        ...
        v.setTextViewText(R.id.mini_site, site?.optString("name") ?: "")
